...
class AdminUiIntegrationTest {
	...
	void adminAppScriptContainsOperationalEndpointsAndRenderers() throws Exception {
		mockMvc.perform(get("/admin/app.js"))
			...
			.andExpect(content().string(containsString("createRefreshContextSnapshot")))
			.andExpect(content().string(containsString("isRefreshContextCurrent")))
			.andExpect(content().string(containsString("createSectionContextSnapshot")))
			.andExpect(content().string(containsString("isSectionContextCurrent")))
			.andExpect(content().string(containsString("state.refreshRequestId += 1")))
			.andExpect(content().string(containsString(": \"0.45\"")))
